...
| `packages/happy-app` | active desktop/Android/browser replacement target | `packages/vibe-app-tauri` | active replacement tree | active | direct Happy reference now flows to `packages/vibe-app-tauri`; new planning and active pipelines should target this package first |
...
| `packages/happy-app/src-tauri/**` | desktop shell behavior reference for the active replacement package | `packages/vibe-app-tauri` | active Wave 10 module set | active | reuse behavior as reference only; do not mutate the existing `packages/vibe-app/src-tauri` path while evolving the active replacement package |
| `packages/happy-app/index.ts` | Happy app runtime entrypoint and bootstrap wiring reference | `packages/vibe-app-tauri` | active Wave 10 module set | active | continuity input only; canonical runtime ownership now follows the active `projects/vibe-app-tauri.md` and `modules/vibe-app-tauri/*` plans |
| `packages/happy-app/app.config.js` | Happy Expo/mobile release config reference | `packages/vibe-app-tauri` | active Wave 10 module set | active | continuity input only; do not treat Expo config as the canonical runtime or release boundary |
| `packages/happy-app/eas.json` | Happy EAS build profile reference | `packages/vibe-app-tauri` | active Wave 10 module set | active | continuity input only; active release and platform work follows the current planning set |
...
| `packages/happy-app/release-*.sh` | Happy release helper script reference | `packages/vibe-app-tauri` | active Wave 10 module set | active | continuity input only; package-local release helpers remain the Vibe implementation path |
...
| `packages/happy-app/sources/app/**` | route and screen reference for the active replacement package | `packages/vibe-app-tauri` | active Wave 10 module set | active | use the current active project and module plans as the execution authority; archived Wave 9 route materials are continuity-only references |
...
| `packages/happy-app/sources/utils/**` | utility and platform-adapter reference for the active replacement package | `packages/vibe-app-tauri` | active Wave 10 module set | active | pure TS utilities may be copied; platform helpers must be rewritten behind explicit current-package adapters |
