...
| `packages/happy-app/sources/sync/**` | sync/domain extraction source for the active replacement package | `packages/vibe-app-tauri` | `shared-core-from-happy` / `session-runtime-and-storage` | planned | sync/domain logic lands package-local first and must be split from React Native/Expo assumptions explicitly |
...
| `packages/happy-app/sources/hooks/**` | state and interaction helper reference for the active replacement package | `packages/vibe-app-tauri` | `shared-core-from-happy` / `session-rendering-and-composer` | planned | only hooks without hard React Native coupling should be ported directly |
...
| `packages/happy-app/sources/constants/**` | constant/token reference for the active replacement package | `packages/vibe-app-tauri` | `shared-core-from-happy` / `mobile-shell-and-navigation` | planned | copy only the constants that support parity and avoid pulling Expo/mobile-only assumptions forward |
