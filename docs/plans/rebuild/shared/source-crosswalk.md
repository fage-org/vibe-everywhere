...
| `packages/happy-app` | mobile/web/desktop app | `packages/vibe-app` | imported Happy tree | done | imported app baseline plus Vibe adaptation layers are implemented and validated |
| `packages/happy-app` | desktop UX and feature reference for the next desktop rewrite | `packages/vibe-app-tauri` | planned desktop rewrite tree | planned | phase-two desktop rewrite uses `vibe-app` and Happy app behavior as the parity reference; new ownership stays parallel to `packages/vibe-app` until promotion |
...
| `packages/happy-app/src-tauri/**` | desktop shell behavior reference for the parallel rewrite | `packages/vibe-app-tauri` | future desktop shell | planned | reuse behavior as reference only; do not mutate the existing `packages/vibe-app/src-tauri` path while bootstrapping the new project |
...
| `packages/happy-app/sources/` | desktop route, state, and component reference for the parallel desktop rewrite | `packages/vibe-app-tauri` | future desktop rewrite tree | planned | extraction during `vibe-app-tauri` lands package-local first; broad cross-package sharing is deferred until the new desktop app proves parity |
...
| `packages/happy-app/sources/app/**` | desktop route and screen reference for the parallel rewrite | `packages/vibe-app-tauri` | `desktop-shell-and-routing` / `session-ui-parity` / `secondary-surfaces` | planned | use current desktop-visible route tree as the parity inventory baseline |
...
| `packages/happy-app/sources/auth/**` | desktop auth/account restore reference and extraction source | `packages/vibe-app-tauri` | `auth-and-session-state` | planned | logic lands package-local first; secure storage and callback handling are reworked behind desktop adapters |
...
| `packages/happy-app/sources/config.ts` | desktop config/env reference | `packages/vibe-app-tauri` | `bootstrap-and-package` | planned | desktop rewrite may reuse concepts but should not inherit Expo/mobile config assumptions directly |
...
| `packages/happy-app/sources/realtime/**` | desktop realtime/session subscription reference | `packages/vibe-app-tauri` | `auth-and-session-state` | planned | only the desktop-needed realtime path should be recreated initially |
...
| `packages/happy-app/sources/sync/**` | desktop sync/domain extraction source | `packages/vibe-app-tauri` | `core-logic-extraction` / `auth-and-session-state` | planned | sync/domain logic lands package-local first and must be split from React Native/Expo assumptions explicitly |
...
| `packages/happy-app/sources/encryption/**` | desktop encryption helper reference | `packages/vibe-app-tauri` | `core-logic-extraction` | planned | may be copied/adapted into the new package if it can be detached from mobile runtime assumptions |
...
| `packages/happy-app/sources/components/**` | desktop UI parity reference and rewrite source | `packages/vibe-app-tauri` | `desktop-shell-and-routing` / `session-ui-parity` / `secondary-surfaces` | planned | component behavior is a parity reference; new implementations are desktop-web-native rather than direct imports |
| `packages/happy-app/sources/modal/**` | modal UI flows | `packages/vibe-app` | imported component tree | done | imported baseline |
| `packages/happy-app/sources/modal/**` | desktop overlay/modal reference | `packages/vibe-app-tauri` | `desktop-shell-and-routing` | planned | modal semantics should be preserved while adopting desktop/web-native focus handling |
...
| `packages/happy-app/sources/hooks/**` | desktop state and interaction helper reference | `packages/vibe-app-tauri` | `core-logic-extraction` / `session-ui-parity` | planned | only hooks without hard React Native coupling should be ported directly |
...
| `packages/happy-app/sources/utils/**` | desktop utility and platform-adapter reference | `packages/vibe-app-tauri` | `core-logic-extraction` / `desktop-platform-adapters` | planned | pure TS utilities may be copied; platform helpers must be rewritten behind desktop adapters |
...
| `packages/happy-app/sources/text/**` | desktop text/localization source | `packages/vibe-app-tauri` | `core-logic-extraction` / `secondary-surfaces` | planned | text assets may be mirrored package-local to keep the old app untouched during early phases |
...
| `packages/happy-app/sources/constants/**` | desktop constant/token reference | `packages/vibe-app-tauri` | `core-logic-extraction` / `desktop-shell-and-routing` | planned | copy only the constants that support parity and avoid pulling Expo/mobile-only assumptions forward |
...
| `packages/happy-app/sources/assets/**` | desktop asset parity source | `packages/vibe-app-tauri` | `bootstrap-and-package` / `desktop-shell-and-routing` / `secondary-surfaces` | planned | copy or mirror only the assets required for desktop parity; keep package-local ownership and build outputs distinct from `packages/vibe-app` |
...
| `packages/happy-app/sources/track/**` | desktop tracking/analytics decision point | `packages/vibe-app-tauri` | `release-and-promotion` | planned | track ownership stays explicit even if telemetry is deferred during early parity work |
...
| `packages/happy-app/sources/types/**` | desktop TS shim/type-helper source | `packages/vibe-app-tauri` | `core-logic-extraction` / `bootstrap-and-package` | planned | package-local declaration files may be copied as needed without creating a shared package |
