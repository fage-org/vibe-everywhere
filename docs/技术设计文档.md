...
| 客户端框架   | Tauri 2                           | 2.10.x         | 跨桌面和移动端的轻量原生应用框架      |
| 前端框架     | Vue 3                             | 3.5.x          | Composition API + `<script setup>`    |
| 前端构建     | Vite                              | 6.x            | 快速开发服务器和构建工具              |
| 状态管理     | Pinia                             | 3.x            | Vue 3 官方推荐状态管理                |
| 前端路由     | Vue Router                        | 4.x            | —                                     |
| CSS          | Tailwind CSS                      | 4.x            | Utility-first CSS 框架                |
| 前端语言     | TypeScript                        | 5.x            | —                                     |
...
- UI 渲染和交互
- Tauri IPC 调用（本地能力：推送通知、扫码等）
- 本地状态管理（Pinia）
- REST API 调用（CRUD 操作）
- WebSocket 连接（接收实时事件推送）
- 响应式布局适配桌面端和移动端
...
项目采用 Mono-repo 组织，Rust 部分使用 Cargo workspace，客户端使用 Tauri 2 + Vue 3。
...
│       └── Cargo.toml
│
├── client/                         # Tauri 2 + Vue 3 客户端
│   ├── src-tauri/                  # Tauri Rust 后端
│   │   ├── src/
│   │   │   ├── lib.rs             # Tauri 插件注册和 setup
...
│   │   ├── stores/                # Pinia stores
│   │   │   ├── auth.ts            # 设备授权状态
│   │   │   ├── sessions.ts        # 会话列表和当前会话
│   │   │   ├── hosts.ts           # 主机和 workspace
│   │   │   ├── archives.ts        # 归档会话
│   │   │   ├── notifications.ts   # 通知偏好
│   │   │   └── websocket.ts       # WebSocket 连接管理
│   │   │
│   │   ├── views/                 # 页面视图（按模块组织）
│   │   │   ├── setup/             # 首次接入：服务器配置、设备授权、主机配对
│   │   │   ├── sessions/          # 会话模块：列表、新建、详情
│   │   │   ├── hosts/             # 主机模块：列表、详情、workspace
│   │   │   ├── archives/          # 归档模块：列表、详情
│   │   │   └── settings/          # 设置模块
│   │   │
│   │   ├── components/            # 可复用组件
...
│   │   │   ├── mobile/            # 手机端特有组件
│   │   │   │   ├── BottomTabBar.vue
│   │   │   │   ├── FloatingNewSessionButton.vue
│   │   │   │   └── SessionActionMenu.vue
│   │   │   └── desktop/           # 桌面端特有组件
│   │   │       ├── SessionWorkbenchLayout.vue
│   │   │       ├── SidebarSessionList.vue
│   │   │       └── ContextInspectorPanel.vue
│   │   │
│   │   ├── composables/           # Vue composables
│   │   │   ├── usePlatform.ts     # 平台检测（desktop/mobile）
│   │   │   ├── useWebSocket.ts    # WebSocket 连接
│   │   │   └── useNotification.ts # 通知处理
│   │   │
│   │   ├── services/              # 外部服务封装
│   │   │   ├── api.ts             # REST API 客户端
│   │   │   └── ws.ts              # WebSocket 客户端
...
桌面端和移动端共用同一套 Vue 3 前端代码，通过以下方式适配差异：
