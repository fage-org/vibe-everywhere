...
function looksLikeFormattedBackupKey(value: string): boolean {
  const trimmed = value.trim();
  return (
    /^[A-Z2-7\s-]+$/i.test(trimmed)
...
export function isValidSecretKey(key: string): boolean {
  try {
    ...
    if (looksLikeFormattedBackupKey(trimmed)) {
      return decodeBase64(parseBackupSecretKey(trimmed), "base64url").length === 32;
    }
    return decodeBase64(trimmed, "base64url").length === 32;
  } catch {
    return false;
  }
}
