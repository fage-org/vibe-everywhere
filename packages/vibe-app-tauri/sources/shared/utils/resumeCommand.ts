...
function buildResumeInvocation(metadata: ResumeCommandMetadata): string | null {
  if (
    (metadata.flavor === "codex" || metadata.flavor === "openai" || metadata.flavor === "gpt")
...
function buildChangeDirectoryCommand(metadata: ResumeCommandMetadata): string | null {
  const location = metadata.path?.trim();
  if (!location) {
    return null;
  }

  return isWindows(metadata)
    ? `Set-Location -LiteralPath ${quotePowerShellPath(location)}`
    : `cd ${quotePosixPath(location)}`;
}
...
export function buildResumeCommandBlock(
  ...
  const lines = changeDirectoryCommand ? [changeDirectoryCommand, invocation] : [invocation];
