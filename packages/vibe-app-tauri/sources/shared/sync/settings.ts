...
export function settingsParse(settings: unknown): Settings {
  ...
  if (!parsed.success) {
    const unknownFields = { ...(settings as Record<string, unknown>) };
    for (const knownField of Object.keys(SettingsSchema.shape)) {
      delete unknownFields[knownField];
    }
  ...
  const migrated = {
    ...parsed.data,
    preferredLanguage:
      parsed.data.preferredLanguage === "zh"
        ? "zh-Hans"
        : (parsed.data.preferredLanguage ?? null),
  };

  const unknownFields = { ...(settings as Record<string, unknown>) };
  for (const knownField of Object.keys(migrated)) {
    delete unknownFields[knownField];
  }

  return { ...settingsDefaults, ...migrated, ...unknownFields };
}
