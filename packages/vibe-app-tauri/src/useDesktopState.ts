...
/**
 * useDesktopState - Desktop state management hook
 *
 * This hook manages the desktop application state including:
 * - Authentication and credentials
 * - Session management
 * - Machine listing
 * - Real-time updates
 *
 * Historical note: Previously named useWave8Desktop, renamed to better reflect
 * its purpose as the desktop state management hook.
 */
export function useDesktopState(activeSessionId?: string | null) {
  const clientRef = useRef<DesktopClient | null>(null);
  ...
  const connectClient = useCallback(
    async (nextCredentials: StoredCredentials, nextServerUrl: string) => {
      const client = await DesktopClient.connect(nextServerUrl, nextCredentials);
