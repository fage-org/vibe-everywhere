 ...
 */
function applyVibeBrandingToString(value: string): string {
    const replacements: Array<[RegExp, string]> = [
        [/happy:\/\/\/terminal\?/g, 'vibe:///terminal?'],
        [/happy:\/\/terminal\?/g, 'vibe:///terminal?'],
        [/happy:\/\/\/account\?/g, 'vibe:///account?'],
        [/happy-agent auth login/g, 'vibe-agent auth login'],
        [/happy daemon status/g, 'vibe daemon status'],
        [/happy codex --resume/g, 'vibe codex --resume'],
        [/happy claude --resume/g, 'vibe claude --resume'],
        [/happy@latest/g, 'vibe@latest'],
        [/Happy Coder/g, 'Vibe'],
        [/Happy CLI/g, 'Vibe CLI'],
        [/Happy Session ID/g, 'Vibe Session ID'],
        [/Happy Server/g, 'Vibe Server'],
        [/Happy Home/g, 'Vibe Home'],
        [/Happy default/g, 'Vibe default'],
        [/Happy server/g, 'Vibe server'],
        [/Happy/g, 'Vibe'],
    ];

    return replacements.reduce((current, [pattern, next]) => current.replace(pattern, next), value);
}

function brandTranslationTree<T>(value: T): T {
    if (typeof value === 'string') {
        return applyVibeBrandingToString(value) as T;
    }
    if (typeof value === 'function' || value === null || value === undefined) {
        return value;
    }
    if (Array.isArray(value)) {
        return value.map((item) => brandTranslationTree(item)) as T;
    }
    if (typeof value === 'object') {
        return Object.fromEntries(
            Object.entries(value as Record<string, unknown>).map(([key, item]) => [
                key,
                brandTranslationTree(item),
            ])
        ) as T;
    }
    return value;
...
const translations: Record<SupportedLanguage, TranslationStructure> = {
    en: brandTranslationTree(en),
    ru: brandTranslationTree(ru),
    pl: brandTranslationTree(pl),
    es: brandTranslationTree(es),
    it: brandTranslationTree(it),
    pt: brandTranslationTree(pt),
    ca: brandTranslationTree(ca),
    'zh-Hans': brandTranslationTree(zhHans),
    'zh-Hant': brandTranslationTree(zhHant),
    ja: brandTranslationTree(ja),
};
